...
!**********************************************************************************************************************!
  use smss_module, only: lusmss, ReadEnsembleInput, WriteEnsembleForces
!**********************************************************************************************************************!
!                                                 End of Modification.                                                 !
!**********************************************************************************************************************!
...
!                                        Modification by Mian Muhammad Faheem.                                         !
!**********************************************************************************************************************!
  !Read SMSS ensemble input.
  call ReadEnsembleInput()
!**********************************************************************************************************************!
!                                                 End of Modification.                                                 !
!**********************************************************************************************************************!
...
!**********************************************************************************************************************!
  ...
  stpeng = (stpeng+consv) * 3.8088e-6_wp
  if (idnode==0) then
    open (unit=lusmss, file='MMS_REPLAY', status='UNKNOWN', action='WRITE', position='APPEND', iostat=code)
      if (code>0) call error (9011)
    write (lusmss,'(A)',iostat=code)        '$MMS_FRAME'
      if (code>0) call error (9012)
    write (lusmss,'(A,F20.14)',iostat=code) '$temperature = ', stptmp
      if (code>0) call error (9012)
    write (lusmss,'(A,F20.14)',iostat=code) '$energy      = ', stpeng
      if (code>0) call error (9012)
    write (lusmss,'(A)',iostat=code)        '$gradients'
      if (code>0) call error (9012)
    close (lusmss)
  end if

  !Write forces for SMSS ensemble atoms.
  if (mxnode>1) call gsync()
  call WriteEnsembleForces()
